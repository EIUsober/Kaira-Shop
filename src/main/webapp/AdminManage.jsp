...
<body>
	<main>
		<c:if test="${editMessage != null}">
			<p id="editMessage" class="alert-success alert"
				style="font-size: 30px">${editMessage}</p>
		</c:if>
		<c:if test="${addMessage != null}">
			<p id="addMessage" class="alert-success alert"
				style="font-size: 30px">${addMessage}</p>
		</c:if>
		<p id="deleteMessage" class="${deleteMessage==null ? "
			": "alert-danger alert" }" style="size: 30px">${deleteMessage}</p>
		<div id="results"></div>
		<div class="row">
							...
							<c:if test="${listProducts != null}">
								<c:forEach items="${listProducts}" var="l">
									<tr>
										...
										<td>${l.getType()}</td>
										<td>${l.getCategory()}</td>
										<td><div class="img-box img-fluid">
								...
								<c:forEach items="${listProduct}" var="l">
									<tr>
										...
										<td><a class="button-like"
											href="AdminControl?mode=editInfo&productId=${l.getProductId()}">
												<i class="fas fa-edit"></i>
										</a> <a class="button-like" href="#"
											onclick="confirmDelete('${l.getProductId()}')"> <i
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="AdminControl?mode=addNewProduct" method="post"
